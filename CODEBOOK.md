This file describes the dataset that was input to the run_analysis.R script and the 
output of that script, the file phonemotionTidy.txt.
...
* Note that in these feature names, from the data set as read in by 
     meansBySubjActRead <- read.table("phonemotionTidy.txt", header=TRUE)
  the characters "()", and "_" have been replaced by ".." and "." respectively.
* t referes to a time-domain variable, f refers to a frequency-domain varible, Acc refers 
to measurements from the phone's accelerometer and Gyro to measurements from the phone's 
gyroscope.
* mean.., ie. mean(), refers to a measurement averaged over a time or frequency window in
the sensor's data, and std.., ie. std(), is the standard deviation in such a set of 
measurements.